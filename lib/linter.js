...
Linter.prototype.resolveRule = function (name) {
  ...
  return Promise.resolve(this.resolver.resolveRule(pkg, ruleName)).then((ruleFactory) => {
    ...
    const ruleObj = this.cachedRules[id] = ruleFactory();
...
Linter.prototype.resolveConfig = function (name) {
  ...
  return Promise.resolve(this.resolver.resolveConfig(pkg, configName)).then((configObj) => {

    if (!configObj) {
      throw new Error(`unknown config <${name}>`);
    }

    const actualConfig = this.cachedConfigs[id] = this.normalizeConfig(configObj, pkg);
 ...
 */
Linter.prototype.resolveRules = function (configObj) {

  return this.resolveConfiguredRules(configObj).then((rulesConfig) => {
...
Linter.prototype.resolveConfiguredRules = function (config) {
  ...
  return Promise.all(
    parents.map((configName) => {
      return this.resolveConfig(configName).then((configs) => {
        return this.resolveConfiguredRules(configs);
      });
    })
  ).then((inheritedRules) => {
    ...
    const allRules = [ ...inheritedRules, overrideRules ].reduce((rules, currentRules) => {
      return {
        ...rules,
        ...currentRules
      };
